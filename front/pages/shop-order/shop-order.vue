				...
				<view class="bg-white margin-sm padding-sm solids-bottom radius text-content" @click="goDetail(item)">
					<view class="flex justify-between">
						<view class="text-bold" style="flex-shrink: 0;">
							<text class="text-red">{{ item.status == 'Unpaid'?'Unpaid':'' }}</text>
							<text class="text-green">{{ item.status == 'Paid'?'Paid':'' }}</text>
							<text class="text-green">{{ item.status == 'Group'?'Group':'' }}</text>
							<text class="text-red">{{ item.status == 'Shipped'?'Shipped':'' }}</text>
							<text class="text-blue">{{ item.status == 'Completed'?'Completed':'' }}</text>
							<text class="text-grey">{{ item.status == 'Cancelled'?'Cancelled':'' }}</text>
							<text class="text-grey">{{ item.status == 'Refunded'?'Refunded':'' }}</text>
						</view>
					...
					<view class="flex justify-end" style="font-size: 24upx;">
						...
						<view v-if="item.status == 'Shipped'">
							<view @tap="onConfirm(item)" class="round cu-btn lines-grey mid  margin-right-sm">确认收货</view>
						</view>
                        <view v-if="item.status == 'Cancelled'||item.status == 'Completed'||item.status == 'Refunded'">
...
<script>
	export default {
		data() {
			return {
				navlist: [
					'All', 'Unpaid', 'Group', 'Paid', 'Shipped' ,'Completed', 'Cancelled', 'Refunded'
				],
				currentIndex: 0,
				pages: 1,
				list: [],
