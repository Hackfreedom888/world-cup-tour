			...
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.yudingdanhao" type="text" placeholder="预订单号" ></input>
					</view>
					<view v-if="queryIndex==1" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.yuyueshijian" type="text" placeholder="预约时间" ></input>
					</view>
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"-8rpx 0px 0px rgba(188,188,226,.9)","padding":"16rpx","margin":"0 0 20rpx","backgroundColor":"#ecebf6","borderRadius":"12rpx","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"8rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiudianyuding','修改')) || (!userid && isAuthFront('jiudianyuding','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiudianyuding','删除')) || (!userid && isAuthFront('jiudianyuding','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.yudingdanhao="";
				this.searchForm.yuyueshijian="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.yudingdanhao){
					params['yudingdanhao'] = '%' + this.searchForm.yudingdanhao + '%'
				}
				if(this.searchForm.yuyueshijian){
					params['yuyueshijian'] = '%' + this.searchForm.yuyueshijian + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiudianyuding`, params);
                } else {
                    res = await this.$api.list(`jiudianyuding`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiudianyuding', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.yudingdanhao){
					searchForm['yudingdanhao'] = '%' + this.searchForm.yudingdanhao + '%'
				}
				if(this.searchForm.yuyueshijian){
					searchForm['yuyueshijian'] = '%' + this.searchForm.yuyueshijian + '%'
				}
				if(this.searchForm.yonghuxingming){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiudianyuding`, searchForm);
                } else {
                    res = await this.$api.list(`jiudianyuding`, searchForm);
                }
				// 如果是第一页数据置空
