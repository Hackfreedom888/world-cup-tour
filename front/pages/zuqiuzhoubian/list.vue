		...
		<view :style='{"width":"100%","padding":"0 0 64rpx","position":"relative","background":"url(http://codegen.caihongy.cn/20221108/581bb1956ffe4c8182127ca1e196e95e.png) fixed","height":"auto"}'>
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.shangpinmingcheng" type="text" placeholder="Name" ></input>
					</view>
					<view v-if="queryIndex==1" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.pinpai" type="text" placeholder="Brand" ></input>
					</view>
					<view v-if="queryIndex==2" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.guige" type="text" placeholder="规格" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"-8rpx 0px 0px rgba(188,188,226,.9)","padding":"16rpx","margin":"0 0 20rpx","backgroundColor":"#ecebf6","borderRadius":"12rpx","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#333","borderRadius":"8rpx 8rpx 0 0","background":"#fff","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>{{product.shangpinmingcheng}}</view>
					<image :style='{"width":"100%","padding":"0","margin":"0","objectFit":"cover","display":"block","height":"300rpx"}' mode="aspectFill" class="uni-product-image" v-if="product.shangpinfengmian.substr(0,4)=='http'" :src="product.shangpinfengmian.split(',')[0]"></image>
					<image :style='{"width":"100%","padding":"0","margin":"0","objectFit":"cover","display":"block","height":"300rpx"}' mode="aspectFill" class="uni-product-image" v-else :src="product.shangpinfengmian?baseUrl+product.shangpinfengmian.split(',')[0]:''"></image>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","margin":"8rpx 0 0 0","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zuqiuzhoubian','修改')) || (!userid && isAuthFront('zuqiuzhoubian','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('zuqiuzhoubian','删除')) || (!userid && isAuthFront('zuqiuzhoubian','删除'))" @click.stop="onDeleteTap(product.id)">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`zuqiuzhoubian`, params);
                } else {
                    res = await this.$api.list(`zuqiuzhoubian`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zuqiuzhoubian', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.guige){
					searchForm['guige'] = '%' + this.searchForm.guige + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`zuqiuzhoubian`, searchForm);
                } else {
                    res = await this.$api.list(`zuqiuzhoubian`, searchForm);
                }
				// 如果是第一页数据置空
