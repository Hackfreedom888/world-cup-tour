	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="评论Content" prop="content">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="评论Content"
					  v-model="ruleForm.content" readonly>
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="评论Content" prop="content">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				content: [
					{ required: true, message: '评论Content不能为空', trigger: 'blur' },
				],
				reply: [
				],
			}
    ...
    info(id) {
      this.$http({
        url: `discusszaixianshoupiao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discusszaixianshoupiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discusszaixianshoupiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
