	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="Gender" prop="xingbie">
					<el-select :disabled="ro.xingbie" v-model="ruleForm.xingbie" placeholder="Gender" >
						<el-option
				...
				<el-form-item :style='{"width":"33.3%","margin":"0 0 20px 0"}' v-else class="input" label="Gender" prop="xingbie">
...
export default {
    ...
    onSubmit() {
      ...
      if((this.ruleForm.yonghuzhanghao.length<3)){
        this.$message.error(`Account长度不能小于3`);
        return
      }
      if((this.ruleForm.yonghuzhanghao.length>10)){
        this.$message.error(`Account长度不能大于10`);
        return
      }


      if((this.ruleForm.mima.length<3)){
        this.$message.error(`Password长度不能小于3`);
        return
      }
      if((this.ruleForm.mima.length>10)){
        this.$message.error(`Password长度不能大于10`);
        return
      }
