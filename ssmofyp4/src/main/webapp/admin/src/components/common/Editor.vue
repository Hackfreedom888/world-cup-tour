...
export default {
  ...
  data() {
    return {
      content: this.value,
      quillUpdateImg: false, // 根据Image上传State来Confirm是否显示loading动画，刚开始是false,不显示
      editorOption: {
        placeholder: "",
      ...
      // serverUrl: `${base.url}sys/storage/uploadSwiper?token=${storage.get('token')}`, // 这里写你要上传的Image服务器Address
      header: {
  ...
  methods: {
    ...
    uploadSuccess(res, file) {
      ...
      if (res.code === 0) {
        ...
        // 插入Image  res.url为服务器Return的ImageAddress
