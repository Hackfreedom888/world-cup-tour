	...
	<view class="w-picker">
		...
		<view class="w-picker-cnt" :class="{'show':showPicker}">
			<view class="w-picker-hd" @touchmove.stop.prevent catchtouchmove="true">
			  <view class="w-picker-btn" @tap="pickerCancel">cancel</view>
			  <view class="w-picker-btn" :style="{'color':themeColor}" @tap="pickerConfirm">Confirm</view>
			</view>
			...
			bindChange(val){
				...
				switch(mode){
					...
					case "range":
						...
						_this.resultStr=`${fyear+'-'+fmonth+'-'+fday+'To'+tyear+'-'+tmonth+'-'+tday}`;
						break;
					case "half":
