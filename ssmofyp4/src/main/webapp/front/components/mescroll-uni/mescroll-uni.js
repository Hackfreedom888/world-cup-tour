...
MeScroll.prototype.extendDownScroll = function(optDown) {
	...
	MeScroll.extend(optDown, {
		...
		afterLoading: null, // 准备结束下拉的回调. Return结束下拉的延时执行时间,默认0ms; 常用于结束下拉之前再显示另外一小段动画,才去隐藏下拉刷新的场景, 参考案例【dotJump】
...
MeScroll.prototype.extendUpScroll = function(optUp) {
	...
	MeScroll.extend(optUp, {
		...
		page: {
			...
			size: 10, // 每页数据的Number
			time: null // 加载第一页数据服务器Return的时间; 防止User翻页时,后台Add了数据从而导致下一页数据重复;
		},
		noMoreSize: 5, // 如果列表已无数据,可设置列表的总Number要大于等于5条才显示无更多数据;避免列表数据过少(比如只有一条数据),显示无更多数据会不好看
		...
		toTop: {
			// 回到顶部按钮,需配置src才显示
			src: null, // Image路径,默认null (建议写成网络图,不必考虑相对路径)
...
MeScroll.prototype.endUpScroll = function(isShowNoMore) {
	if (isShowNoMore != null) { // isShowNoMore=null,不处理下拉State,下拉刷新的时候调用
		if (isShowNoMore) {
 ...
 * systime: 服务器时间(可空);用来解决这个小问题:当准备翻下一页时,数据库Add了几条记录,此时翻下一页,前面的几条数据会和上一页的重复;这里传入了systime,那么upCallback的page.time就会有值,把page.time传给服务器,让后台过滤新加入的那几条记录
 */
