...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusszuqiuzhoubianCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusszuqiuzhoubianCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusszuqiuzhoubian');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusszuqiuzhoubian');
			...
			// 获取Details
			async init(){
				let res = await this.$api.info('discusszuqiuzhoubian', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusszuqiuzhoubian', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusszuqiuzhoubian/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
