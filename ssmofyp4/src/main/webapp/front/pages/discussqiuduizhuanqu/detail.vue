...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussqiuduizhuanquCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussqiuduizhuanquCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussqiuduizhuanqu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussqiuduizhuanqu');
			...
			// 获取Details
			async init(){
				let res = await this.$api.info('discussqiuduizhuanqu', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussqiuduizhuanqu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussqiuduizhuanqu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
