				...
				<view class="bg-white margin-sm padding-sm solids-bottom radius text-content" @click="goDetail(item)">
					<view class="flex justify-between">
						<view class="text-bold" style="flex-shrink: 0;">
							<text class="text-red">{{ item.status == 'Paid'?'Paid':'' }}</text>
							...
							<text class="text-grey">{{ item.status == '已cancel'?'已cancel':'' }}</text>
					...
					<view class="bg-white margin-xs padding-xs  radius text-content">
						<view class="flex padding-top-sm solids-top">
							...
							<view class="text-content padding-left" style="flex-grow: 1;">
								...
								<view class="flex justify-end">Customer：{{item.consignee}}</view>
								<view class="flex justify-end">Contact：{{item.tel}}</view>
								<view class="flex justify-end">Address：{{item.address}}</view>
								<view class="flex justify-end">Time：{{item.addtime}}</view>
								<view class="flex justify-end">Tips：{{item.remark}}</view>
							</view>
						</view>
					...
					<view class="flex justify-end" style="font-size: 24upx;">
						<view v-if="item.status == 'Paid'">
							<view @tap="pay(item)" class="round cu-btn lines-grey mid  margin-right-sm">支付</view>
							<view @tap="cancel(item)" class="round cu-btn lines-grey mid  margin-right-sm">cancel订单</view>
						</view>
						...
						<view v-if="item.status == '拼团中'">
							<view @tap="cancelGroup(item)" class="round cu-btn lines-grey mid  margin-right-sm">cancel拼团</view>
						</view>
                        ...
                        <view v-if="item.status == '已cancel'||item.status == '已完成'||item.status == '已退款'">
                            <view @tap="del(item.id)" class="round cu-btn lines-grey mid  margin-right-sm">Delete订单</view>
                        </view>
...
<script>
	export default {
		data() {
			return {
				navlist: [
					'全部', 'Paid', '拼团中', '已支付', '已发货' ,'已完成', '已cancel', '已退款'
				],
				currentIndex: 0,
				pages: 1,
				list: [],
