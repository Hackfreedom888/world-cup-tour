...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussqiuxingzhuanquCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussqiuxingzhuanquCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussqiuxingzhuanqu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussqiuxingzhuanqu');
			...
			// 获取Details
			async init(){
				let res = await this.$api.info('discussqiuxingzhuanqu', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussqiuxingzhuanqu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussqiuxingzhuanqu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
