...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusssaishizhiboCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusssaishizhiboCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusssaishizhibo');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusssaishizhibo');
			...
			// 获取Details
			async init(){
				let res = await this.$api.info('discusssaishizhibo', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusssaishizhibo', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusssaishizhibo/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
