	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusssaishizhibo`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusssaishizhibo`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusssaishizhibo`, this.ruleForm);
						}else{
							await this.$api.add(`discusssaishizhibo`, this.ruleForm);
						}
						uni.setStorageSync('discusssaishizhiboCleanType',true);
